...
要想用[quantmod](http://www.quantmod.com)进行数据分析，第一步自然得把数据导入到 R 里面。将数据导入[R](http://r-project.org)里面的著名的教程是[数据导入与导出](http://cran.r-project.org/doc/manuals/r-release/R-data.pdf)。里面对大部分关于导入和导出数据的内容都讲的很明晰，读者可以直接去看，这里不再赘述。

下面只说一下针对[quantmod](http://www.quantmod.com)特有的一些数据导入和导出的做法。
...
基于 getSymbols() 函数很容易从前文提到的几个数据库中获取上市公司股票的日交易数据。如果想获取中国移动通讯公司的日交易数据，我们可以输入下面的代码：
